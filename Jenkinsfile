pipeline {
  agent {
    docker {
      image 'node:12'
      args '-i -d --privileged --network jenkins-blue-ocean-tutorial_mynet -v /var/jenkins_home:/var/jenkins_home -v /usr/bin/docker:/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock'
    }
